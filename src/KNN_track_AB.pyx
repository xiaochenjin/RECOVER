...
cpdef tuple compute(list atom_infor_list1,list atom_infor_list2,list cell_geometry,list true_kNN_infor_list,int K_raw,int N_bin,list neigh_dis,float lc,str direction):
	...
	for i in range(N1_total):
		...
		for j in range(N2_total):
			...
			if (k<N_bin):
				for n_shell in range(K_raw):
					if index_j in true_kNN_index[n_shell]: M_truekNN_totlist[n_shell][k] += 1
	...
	for l in range(N_bin):
		...
		for n_shell in range(K_raw):
			...
			density_R=(M_truekNN_list[l]/N1_total)/dV
