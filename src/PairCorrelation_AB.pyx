...
cpdef tuple compute(list atom_infor_list1,list atom_infor_list2,list cell_geometry,int N_bin,str direction):
	...
	for l in range(N_bin):
		...
		if direction == 'xy':
			dV = 2*3.14*R_list[l]*lz*dr #equivalent to as 3.14*((R_list[l]+dr)**2-R_list[l]**2)*lz when dr is small
		if direction == 'r': 
			dV = 4*3.14*R_list[l]*R_list[l]*dr
		density_R=(M[l]/N1_total)/dV
